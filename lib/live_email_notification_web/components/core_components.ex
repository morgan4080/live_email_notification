defmodule LiveEmailNotificationWeb.CoreComponents do
  @moduledoc """
  ...
  import LiveEmailNotificationWeb.Gettext
  ...
  attr :type, :string,
    ...
    values: ~w(checkbox color date datetime-local email file hidden month number password
               range radio search select selectkv tel text textarea time url week)
  ...
  def input(%{type: "selectkv"} = assigns) do
    ~H"""
    <div phx-feedback-for={@name}>
      <.label for={@id}><%= @label %></.label>
      <select
        ...
        {@rest}
      >
        <option :for={option <- @options} value={option.id} selected={option.selected}><%= option.name %></option>
      </select>
      <.error :for={msg <- @errors}><%= msg %></.error>
    </div>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LiveEmailNotificationWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LiveEmailNotificationWeb.Gettext, "errors", msg, opts)
    end
  end
